...
from .verdict import build_attack_path
...
def report_payload(state: InvestigationState) -> dict:
    ...
    return {
        ...
        "verdict": verdict,
        "entities": [x.model_dump(mode="json") for x in state.entities],
        "evidence": [x.model_dump(mode="json") for x in state.evidence],
        "attack_path": build_attack_path(state),
        "facts": [x.model_dump(mode="json") for x in state.facts],
        "findings": [x.model_dump(mode="json") for x in state.findings],
        "hypotheses": [x.model_dump(mode="json") for x in state.hypotheses],
        "evidence_roles": [x.model_dump(mode="json") for x in state.evidence_roles],
        "interpretations": [x.model_dump(mode="json") for x in state.interpretations],
        "coverage": {k: v.model_dump(mode="json") for k, v in state.coverage.items()},
        "analysis_obligations": [x.model_dump(mode="json") for x in state.analysis_obligations],
        "evidence_gaps": [x.model_dump(mode="json") for x in state.evidence_gaps],
        "unresolved_gaps": [x.model_dump(mode="json") for x in state.evidence_gaps if x.status != "resolved"],
        "tool_calls": [x.model_dump(mode="json") for x in state.tool_calls],
        "planner_decisions": [x.model_dump(mode="json") for x in state.planner_decisions],
        "tool_scores": [x.model_dump(mode="json") for x in state.tool_scores],
        "evidence_packs": [x.model_dump(mode="json") for x in state.evidence_packs],
        "repair_actions": [x.model_dump(mode="json") for x in state.repair_actions],
        "active_scenarios": list(state.active_scenarios),
        "scope": state.scope.model_dump(mode="json"),
        "scope_expansions": [item.model_dump(mode="json") for item in state.scope_expansions],
        "verdict_validation": {
            "status": "PASS" if not state.verdict_validation_errors else "FAIL",
            "errors": list(state.verdict_validation_errors),
        },
        "analysis_notes": list(dict.fromkeys(
            note for obligation in state.analysis_obligations for note in obligation.limitations
        )),
        "limitations": verdict.get("limitations", []) if verdict else [],
    }
...
def write_reports(state: InvestigationState, output_dir: Path, expected: dict | None = None) -> tuple[Path, Path, Path]:
