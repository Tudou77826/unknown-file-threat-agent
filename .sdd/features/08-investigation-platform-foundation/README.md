...
实施步骤 01—07 已完成。在线研判使用四个数据能力工具和正式报告发布校验；运行、事件、审计和发布产物可在服务重启后恢复。展示页面通过正式调查 API 读取运行事件、`InvestigationReport` 和独立的 `ResponsePlan`，不再按预置数据集补写结论。首版使用 SQLite 完成业务闭环并保持存储可替换边界；百万级活动仅作为索引、分页和批量写入的设计假设，不在本 Feature 做容量实测。确定性路径只承担离线测试，不构成版本兼容承诺。
