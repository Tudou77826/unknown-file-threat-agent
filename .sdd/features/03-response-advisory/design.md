...
`ResponseGraph` 是由案件父图原生挂载的 LangGraph 子图。知识通过 `KnowledgeRetrievalPort` 获取，当前使用 Null Adapter；资产与业务上下文通过 `ResponseContextPort` 获取，当前使用 Null Provider 并把缺失项写入 `missing_context`。研判与处置模型分别配置，未指定专用模型时才回退到公共 `MODEL_NAME`。
