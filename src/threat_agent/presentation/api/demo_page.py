...
function renderEvent(raw){{const e=normalizeEvent(raw);if(['run','graph'].includes(e.kind)){{setStage('understand');$('thinking').textContent='正在理解告警、资产与数据条件';return}}if(e.kind==='tool'){{setStage('verify');const d=e.details||{{}};const domain=d.activity_type;if(domain&&!seenDomains.has(domain)){{seenDomains.add(domain);$('evidence-count').textContent=String(seenDomains.size)}}addFinding('evidence','数据核验',domain?`已查询${{activityZh[domain]||domain}}，返回 ${{d.returned_count??'若干'}} 条活动记录。`:`已完成一项数据核验：${{d.tool_name||'调查工具'}}。`);$('thinking').textContent='正在核对活动记录与实体关联';return}}if(['thinking','decision','repair','validation'].includes(e.kind)){{$('thinking').textContent=e.message.includes('处置')?'研判已移交处置模型，正在评估业务影响':'AI 正在综合本次查询返回的数据';return}}if(e.kind==='verdict'){{setStage('conclude');$('thinking').textContent='研判模型已完成证据交叉验证';$('current-verdict').textContent=e.message.replace('研判结论：','');addFinding('verdict','研判形成',e.details?.summary||e.message);return}}if(e.kind==='report'){{setStage('conclude');addFinding('verdict','报告生成','正式研判报告已经通过引用与调查范围校验。');return}}if(e.kind==='response'){{setStage('advise');$('thinking').textContent='处置模型正在检查审批约束和业务影响';addFinding('response','处置形成',`已形成 ${{e.details?.action_types?.length||0}} 项处置建议。`);return}}if(e.kind==='error')addFinding('verdict','运行异常',e.message)}}
...
function statements(items){{return (items||[]).map(x=>x.text).filter(Boolean).join('；')||'报告未声明'}}
function renderOutputs(job){{const report=job.investigation_report;const verdict=report?.verdict;const plan=job.response_plan;const findings=report?.key_findings||[];$('report-verdict').textContent=VERDICT_LABELS[verdict?.level]||verdict?.level||'未形成结论';$('report-summary').textContent=report?.executive_summary||verdict?.summary||'未生成正式研判报告';const confidences=findings.map(f=>Number(f.confidence||0));$('report-confidence').textContent=confidences.length?`最高置信度 ${{Math.round(Math.max(...confidences)*100)}}%`:'';$('report-threat').textContent=threatZh[verdict?.threat_type]||verdict?.threat_type||'待确认';$('report-assets').textContent=(report?.asserted_host_refs||[]).join('、')||'报告未声明';$('report-coverage').textContent=`${{report?.query_boundary_refs?.length||0}} 个查询边界`;$('report-ref-count').textContent=`${{report?.supporting_evidence_refs?.length||0}} 条证据引用`;$('report-impact').textContent=statements(report?.current_situation);$('report-counter').textContent=statements(report?.counter_evidence);$('report-scope').textContent=statements(report?.affected_scope);$('report-limitations').textContent=(report?.limitations||[]).join('；')||'报告未声明额外限制';$('report-evidence').replaceChildren(...findings.map(f=>{{const e=document.createElement('div');e.className='evidence-item';e.innerHTML=`<b>${{safe(f.finding_type)}}</b><span>${{safe(f.statement)}} · 置信度 ${{Math.round(Number(f.confidence||0)*100)}}% · 引用 ${{f.evidence_refs?.length||0}} 条证据</span>`;return e}}));$('response-status').textContent=statusZh[plan?.status]||plan?.status||'尚未生成';$('response-meta').innerHTML=`<span>建议 ${{plan?.actions?.length||0}} 项</span><span>剩余风险 ${{plan?.residual_risk?.length||0}} 项</span><span>生产动作未执行</span>`;$('response-actions').replaceChildren(...(plan?.actions||[]).map(a=>{{const e=document.createElement('div');e.className='action-item';e.innerHTML=`<b>${{safe(actionZh[a.action_type]||a.action_type)}}</b><span>${{safe(a.rationale)}}<br>业务影响：${{safe(a.expected_impact||'待评估')}}<br>审批：${{safe(a.approval_class||'按策略执行')}}</span>`;return e}}));$('outputs').classList.add('show')}}
