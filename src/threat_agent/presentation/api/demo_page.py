...
function renderOutputs(job){{const report=job.investigation_report;const verdict=report?.verdict;const plan=job.response_plan;const keyEvidence=report?.key_evidence||[];$('report-verdict').textContent=VERDICT_LABELS[verdict?.level]||verdict?.level||'未形成结论';if(report?.publication_status==='fallback'){{$('report-verdict').textContent+=' · 兜底发布';$('report-summary').textContent='重要：本次报告未通过证据接地校验，系统不保留任何安全结论或事实断言，仅发布确定性证据不足兜底报告。'+(report?.executive_summary||'');}}else{{$('report-summary').textContent=report?.executive_summary||verdict?.summary||'未生成正式研判报告';}}$('report-threat').textContent=threatZh[verdict?.threat_type]||verdict?.threat_type||'待确认';$('report-assets').textContent=(report?.asserted_host_refs||[]).join('、')||'报告未声明';$('report-coverage').textContent=`${{report?.query_boundary_refs?.length||0}} 个查询边界`;$('report-ref-count').textContent=`${{report?.supporting_evidence_refs?.length||0}} 条证据引用`;$('report-impact').textContent='';fillList('report-impact',statements(report?.current_situation),'报告未声明');$('report-counter').textContent='';fillList('report-counter',statements(report?.counter_evidence),'报告未声明');$('report-scope').textContent='';fillList('report-scope',statements(report?.affected_scope),'报告未声明');const limits=(report?.limitations||[]);fillList('report-limitations',limits,'报告未声明额外限制');$('repair-traces').style.display='none';$('report-evidence').replaceChildren(...keyEvidence.map(s=>{{const e=document.createElement('div');e.className='evidence-item';e.innerHTML=`<span>${{safe(s.text)}} · 引用 ${{s.supporting_refs?.length||0}} 条证据</span>`;return e}}));$('response-status').textContent=statusZh[plan?.status]||plan?.status||'尚未生成';$('response-meta').innerHTML=`<span>建议 ${{plan?.actions?.length||0}} 项</span><span>剩余风险 ${{plan?.residual_risk?.length||0}} 项</span><span>生产动作未执行</span>`;$('response-actions').replaceChildren(...(plan?.actions||[]).map(a=>{{const e=document.createElement('div');e.className='action-item';e.innerHTML=`<b>${{safe(actionZh[a.action_type]||a.action_type)}}</b><span>${{safe(a.rationale)}}<br>业务影响：${{safe(a.expected_impact||'待评估')}}<br>审批：${{safe(a.approval_class||'按策略执行')}}</span>`;return e}}));$('outputs').classList.add('show')}}
